...
export function ThemeToggle() {
  ...
  return (
    <button
      ...
      className={`group relative inline-flex h-10 w-16 cursor-pointer items-center rounded-full border border-slate-200 bg-white p-1 text-slate-700 shadow-sm transition-all duration-300 hover:-translate-y-0.5 hover:shadow-lg hover:shadow-indigo-500/10 focus:outline-none focus:ring-2 focus:ring-indigo-400 dark:border-white/10 dark:bg-white/10 dark:text-slate-100 dark:hover:bg-white/15 ${isAnimating ? "theme-toggle-pop" : ""}`}
    >
      {isAnimating && (
        <span className="theme-toggle-orbit pointer-events-none absolute inset-0 rounded-full border border-indigo-300/60 dark:border-violet-300/50" />
      )}
      <span className="absolute left-2 text-amber-500 transition-opacity duration-300 dark:opacity-45">
        <Sun className="h-3.5 w-3.5" aria-hidden="true" />
      </span>
      <span className="absolute right-2 text-violet-500 opacity-45 transition-opacity duration-300 dark:opacity-100 dark:text-indigo-200">
      ...
      <span
        className={`relative z-10 flex h-8 w-8 items-center justify-center rounded-full shadow-md transition-all duration-500 ease-out ${
          theme === "dark"
            ? "translate-x-6 rotate-[360deg] bg-slate-950 text-indigo-200 shadow-violet-950/40"
